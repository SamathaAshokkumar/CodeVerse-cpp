...
void insert_end(int pid,int at,int bt,int check_bt)
{
     struct srt *temp=(struct srt*)malloc(sizeof(struct srt));
              struct srt *prev=(struct srt*)malloc(sizeof(struct srt));
              temp->pid=pid;
   ...
   {
    prev=front;
    while(prev->succ!=NULL && prev->at<=check_bt)
    {
       prev=prev->succ;
    }
     if(prev->succ==NULL)
...
int btime=0;
  while(temp!=NULL )
  {
    if(count==0)
    {
        ...
        if(idle_time!=execution_time)
        { 
            execution_time=execution_time+quantum+idle_time;
        }
        else
        {
            execution_time=quantum+idle_time;
        }

          btime=temp->bt-quantum;
          ...
          else
          {
            //tat++;
            //avg_tat++;
            //exec_end=exec_end+quantum;
            idle_time=idle_time+quantum;
            check_bt=check_bt+quantum;
            //c=bt;
            deletion();
            insert_end(temp->pid,temp->at,btime,check_bt);
            //displayh_r();
